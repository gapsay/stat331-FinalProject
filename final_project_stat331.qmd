...
$$ \hat{y} = `r round(gdp_lex_lm$coeff[1], 2)` + `r round(gdp_lex_lm$coeff[2], 2)`x $$

corresponds to the scatter plot above of Life Expectancy vs. log GDP per Capita. $\hat{y}$ represents the predicted life expectancy (in years) and the $x$ represents the log-transformed GDP per capita. When the log-transformed GDP is 0 dollars, the estimated life expectancy is `r round(gdp_lex_lm$coeff[1], 2)` years. However, this value is not meaningful since real-world GDP per capita values are always positive, making a log-transformed value of zero unrealistic. The coefficient of the slope, `r round(gdp_lex_lm$coeff[2], 2)`, represents the expected change in life expectancy for a percentage change in GDP per capita. In other words, a 1% increase in GDP per capita leads to an increase of approximately `r round(gdp_lex_lm$coeff[2], 2)*0.01` years in life expectancy.
