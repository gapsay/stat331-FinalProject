...
$$ \hat{Life Expectancy} = `r round(models$estimate[1], 2)` + `r round(models$estimate[2], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[1], 2)` represents the predicted life expectancy. 
...
$$ \hat{Life Expectancy} = `r round(models$estimate[3], 2)` + `r round(models$estimate[4], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[3], 2)` represents the predicted life expectancy. 
...
$$ \hat{Life Expectancy} = `r round(models$estimate[5], 2)` + `r round(models$estimate[6], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[5], 2)` represents the predicted life expectancy. 
...
$$ \hat{Life Expectancy} = `r round(models$estimate[7], 2)` + `r round(models$estimate[8], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[7], 2)` represents the predicted life expectancy. 
...
$$ \hat{Life Expectancy} = `r round(models$estimate[9], 2)` + `r round(models$estimate[10], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[9], 2)` represents the predicted life expectancy. 
...
$$ \hat{Life Expectancy} = `r round(models$estimate[11], 2)` + `r round(models$estimate[12], 2)`*\log_{10}(GDP) $$
...
Although unrealistic, when $\log_{10}(GDP)$ is equal to zero, the intercept of `r round(models$estimate[11], 2)` represents the predicted life expectancy. 
:::
...
p2 <- ggplot(full_data, aes(x = avg_gdp, y = avg_simulated_life_expectancy, color = Continent)) +
  ...
  labs(title = "Simulated Data: GDP vs Life Expectancy",
      x = expression(log[10]("GDP per Capita")),
      y = "Simulated Life Expectancy (years)") +
  theme(
    plot.title = element_text(face = "bold", size = 20),
    axis.title = element_text(size = 16),
    axis.text = element_text(size = 14)
