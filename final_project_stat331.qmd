...
```{r}
continents <- read_csv("./country_by_continents.csv")

# Check which countries in gdp_lex don't match with continents
missing_countries <- gdp_lex |>
  distinct(country) |>
  anti_join(continents, by = c("country" = "Country"))

# Check which countires in continents don't match with gdp_lex
countries_not_in_gdp <- continents |>
  distinct(Country) |>
  anti_join(gdp_lex, by = c("Country" = "country"))

# Create a mapping dictionary from continent countries to gdp_lex countries
...
# Create modified continents dataset with gdp_lex country names as standard
continents_modified <- continents |>
...
gdp_lex_with_continents <- left_join(gdp_lex, continents_modified, by = c("country" = "Country"))
